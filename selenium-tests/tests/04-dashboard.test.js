...
// Dashboard screen E2E tests: protected route redirect checks, layout, DOM stability, no-crash check
...
describe('[DASHBOARD] Dashboard Screen Tests', function () {
  ...
  it('TC031: Navigate to /dashboard redirects unauthenticated user', async function () {
    const url = await driver.getCurrentUrl();
    assert.ok(url, 'URL should be defined');
  });

  it('TC032: Dashboard route access does not crash (no Vite error overlay)', async function () {
    const overlays = await driver.findElements(By.css('vite-error-overlay'));
    assert.strictEqual(overlays.length, 0, 'Vite error overlay on dashboard redirect');
  });

  it('TC033: Dashboard page body is visible after redirect', async function () {
    const body = await driver.findElement(By.tagName('body'));
    assert.ok(await body.isDisplayed(), 'Body not visible after dashboard redirect');
  });

  it('TC034: React root div is still mounted after redirect', async function () {
    const root = await driver.findElement(By.id('root'));
    assert.ok(root, 'React root div not found after redirect');
  });

  it('TC035: Document title is set (not empty)', async function () {
    const title = await driver.getTitle();
    assert.ok(title.length > 0, 'Document title is empty');
  });

  it('TC036: Document readyState is complete', async function () {
    const state = await driver.executeScript('return document.readyState');
    assert.strictEqual(state, 'complete', 'Document readyState is not complete');
  });

  it('TC037: No Javascript errors or console crash on redirect', async function () {
    const result = await driver.executeScript('return 1 + 1');
    assert.strictEqual(result, 2, 'JS execution failed');
  });

  it('TC038: URL is within valid application domain after redirect', async function () {
    const url = await driver.getCurrentUrl();
    assert.ok(url.includes(BASE.replace('https://', '').replace('http://', '').split('/')[0]), 'URL is outside app domain');
  });

  it('TC039: App shell divs are present in page source', async function () {
    const divs = await driver.findElements(By.tagName('div'));
    assert.ok(divs.length > 0, 'App shell lacks div containers');
  });

  it('TC040: Direct link redirect behavior completes within 5 seconds', async function () {
    await driver.get(`${BASE}/#/dashboard`);
